...
nome <- c("raio", "rex", "quack", "pirata", "garfield")
idade <- c(20, 14, 7, 5, 12)
especie <- c("cavalo", "cachorro", "pato", "papagaio", "gato")
alimentacao <- c(20.5, 5.7, 3.0, 1.5, 6.0)

my_farm <- data.frame(nome, idade, especie, alimentacao, stringsAsFactors = F)

my_farm$idade <- as.integer(my_farm$idade) # transformar a coluna idade em 
                                           # interger
